...
const cartSliderLine = document.querySelector('.cart-slider-line');
const cartSliderLeftBtn = document.querySelector('.left-cart-btn');
const cartSliderRightBtn = document.querySelector('.right-cart-btn');
// navigation
...
class Products {
  async getProducts() {
    ...
    const products = result.items.map((item) => {
      const {
        sys: { id },
        fields: { brand, title, price, description },
      } = item;
...
class UI {
  displayFilterBtns(products) {
    ...
    filterBtnsArea.innerHTML = brands
      .map((brand) => {
  ...
  checkEmptyCart() {
    if (!cartItems.length) {
      ...
      cartSliderLine.innerHTML = '<div class="empty-cart">No items...</div>';
      return;
    }
    const div = document.querySelectorAll('.empty-cart');
    if (div.length > 0) {
  ...
  addCartItem(cartItem) {
    ...
    articleSlider.setAttribute('data-id', cartItem.id);
    articleSlider.classList.add('cart-product');
    articleSlider.innerHTML = `<div class="cart-image-wrapper">
                                  <img src="${cartItem.images[0]}" alt="${cartItem.title}">   
                              </div>
                              <h3>${cartItem.title}</h3>
                              <h4>$${cartItem.price}</h4>`;
    cartSliderLine.append(articleSlider);
    this.resizeSlider();
  }
  ...
  removeCartSliderItem(id) {
    const cart = document.querySelector(`.cart-product[data-id='${id}']`);
    cart.remove();
    this.resizeSlider();
    countSlide = 0;
    this.rollCartSlider();
  ...
  startAPP() {
    ...
    sctollLinks.forEach((link) =>
      link.addEventListener('click', this.setScrollingLinks.bind(this))
    );
    // Launch product info
    this.setProductInfo();
    // Launch slider
  ...
  setMenuToggle() {
    ...
    if (menuToggleBtn.classList.contains('anim')) {
      ...
      navMenu
        .querySelectorAll('a')
        .forEach((elem) => (menuHeight += elem.offsetHeight));
    ...
    buttons.forEach((button) => {
      button.addEventListener('click', () => {
        ...
        productInfo.innerHTML = `<button class="info-close-btn">
                                        ...
                                        <div class="info-images-container">${product.images
                                          .map(
                                            (item) =>
  ...
  setProductInfo() {
    ...
    productInfo.addEventListener('click', (event) => {
      ...
      if (event.target.closest('.info-images-container > *')) {
        ...
        const images = [
          ...document.querySelectorAll('.info-images-container > *'),
        ];
  ...
  setSlider() {
    ...
    rightSliderBtn.addEventListener('click', () => {
      ...
      slides.forEach(
        (slide) => (slide.style.transform = `translateX(-${slideStep * 100}%)`)
      );
    });
  ...
  resizeSlider() {
    const length = cartItems.length < 3 ? 3 : cartItems.length;
    widthSlider = cartSlider.offsetWidth;
    cartSliderLine.style.width =
      ((widthSlider - 64) / 3) * length + 32 * (length - 1) + 'px';
    cartSliderLine.style.height = 'auto';
  }

  setCartSlider() {
    this.resizeSlider();
    window.addEventListener('resize', () => {
      this.resizeSlider(), this.rollCartSlider();
    });
    cartSliderRightBtn.addEventListener('click', () => {
      countSlide++;
      if (countSlide >= cartItems.length - 2) {
        countSlide = 0;
      }
      this.rollCartSlider();
    });
    cartSliderLeftBtn.addEventListener('click', () => {
      countSlide--;
      if (countSlide < 0) {
        countSlide = Math.max(cartItems.length - 3, 0);
      }
      this.rollCartSlider();
    });
  }
  ...
  rollCartSlider() {
    cartSliderLine.style.transform =
      'translate(-' + countSlide * ((widthSlider - 64) / 3 + 32) + 'px)';
  }
}
